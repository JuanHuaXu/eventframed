...
func TestDeterministicAvailabilityAndPackingReplay(t *testing.T) {
	...
	packet, err := runtime.Recall(context.Background(), model.RecallRequest{
		...
		Embedding: []float32{1, 0, 0, 0, 0, 0, 0, 0}, EmbeddingModel: "feature-hash-v1:d8:repr=eventframe-5w1h-v1", AsOf: fixture.AsOf,
	...
	if packet.BayesianShadow.JournalID == "" || !packet.BayesianShadow.JournalDurable {
		t.Fatalf("Bayesian shadow journal missing: %+v", packet.BayesianShadow)
	}
